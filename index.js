// Accessing the HTML Element using GetElementById DOM method
...
// fetch movies with help of async await modern es8 features
...
// extracting and sorting the result from api by year and genre
function populateFilters() {
  ...
  filterYearSelect.innerHTML = '<option value=""></option>' + years.map(year => `<option value="${year}">${year}</option>`).join('');
...
function displayMovies(movies) {
  const moviesHTML = movies.map(movie => `
    <div class="movie" data-id="${movie.imdbID}"> <img src="${movie.Poster}" alt="${movie.Title} Poster"><h3>${movie.Title}</h3><h4>${movie.Year}</h4>
      <p>Genre: ${movie.Genre}</p> <p>Language: ${movie.Language}</p>
    </div>
  `).join("");
